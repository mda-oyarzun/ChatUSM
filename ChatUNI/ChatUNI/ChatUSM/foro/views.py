...
def perfil_editar(request, user_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            user.sede = form.cleaned_data.get('sede')
            user.carrera = form.cleaned_data.get('carrera')
            user.save()
            messages.success(request, 'Información del perfil actualizada.')
            return redirect('admin_usuarios')
        else:
            messages.error(request, 'Ha ocurrido un error. Por favor revisa el formulario.')
...
def user_login(request):
    if request.method == 'POST':
        ...
        if user is not None:
            ...
            return render(request, 'login.html', {'error': 'Correo y/o Contraseña inválidos. Revisa que los datos sean correctos.'})
    else:
